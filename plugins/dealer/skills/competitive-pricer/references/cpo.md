...
Check before any MCP call. If the user says "this one is certified" / "it's a CPO unit" / "I'm pricing a certified Accord", CPO is **confirmed**. If they say "not CPO" / "this one isn't certified", it's **explicitly non-CPO**. Trust the user — they're looking at the window sticker.
...
Fires only in W3 (trade-in history), only when `cpo_ever == True` AND the current (most recent, post-sort) listing has `is_certified == False` (explicit 0, NOT None — absent is "unknown", not "confirmed non-CPO"). `parse_history.py` sorts desc internally so `listings[0]` is always the most recent regardless of upstream ordering.
...
## CPO Premium + Net Margin (computed from MarketCheck Price values, in `comp_stats.py`, NOT hand-math)

> *Naming clarification.* `CPO Premium` and `Net Margin from CPO` are NOT
> MarketCheck products — they are market/dealer concepts. The `MarketCheck`
> prefix in this skill is reserved for the values that come **directly** from
> the `predict_price_with_comparables` MCP call (the `MarketCheck Price`
> scalar and its comp counts/distributions). Premium is computed *from* MC
> Price values; that doesn't make Premium a MarketCheck product. Render
> labels for Premium / Net Margin are unprefixed; surrounding context (the
> MC Price lines they sit beneath) makes the source obvious.

Pipe each of the four parse_predict.py outputs (one per role) into
`build_comp_stats_input.py` via the four `--{nocpo,cpo}-{primary,context}-parsed
<path>` flags. The builder reads `marketcheck_price` + comp counts + price stats
from each parsed file and packs them into `marketcheck_predict_input` on the
comp_stats stdin. `comp_stats.py` then emits a consolidated `marketcheck_predict`
block:

```json
marketcheck_predict = {
  "primary_label":      "Franchise" | "Independent",   // mapped from session.dealer_type_title
  ...
  "cpo_primary":        { ... } | null,
  "cpo_context":        { ... } | null,
  "premium_primary":    cpo_primary.marketcheck_price - nocpo_primary.marketcheck_price,    // null if either role missing
  "premium_context":    cpo_context.marketcheck_price - nocpo_context.marketcheck_price,    // null if either role missing
  ...
  "certification_cost": profile.dealer.cpo_certification_cost,         // dealer-sourced, NOT MarketCheck
  "net_margin_primary": premium_primary - certification_cost            // Net Margin from CPO (= Premium − Cert Cost)
}
```

The renderer reads `marketcheck_predict` verbatim. Never hand-compute the
premium / net margin — that path was the source of v4/v5 inline `python3 -c`
attempts that the harness denies. Premium and net margin are MarketCheck-derived;
the certification cost is the dealer's input from the profile.
...
Note: none of these line labels carry the `MarketCheck` prefix.
`CPO Premium` and `Net Margin from CPO` are derived market/dealer concepts
(computed from MarketCheck Price values, but not themselves MarketCheck
products). `Certification Cost (you):` is dealer-sourced from
`profile.dealer.cpo_certification_cost`. Only the upstream `MarketCheck Price`
lines (in the Your Price Position block) carry the prefix because they are
direct outputs of the `predict_price_with_comparables` MCP call.

`marketcheck_predict.premium_primary` and `premium_context` are `null` when
their respective CPO role is missing (CPO branch didn't run, or a prediction
degraded). When either is null, skip the CPO Premium block entirely.
The non-CPO roles' MarketCheck Price lines in `Your Price Position` still render
whatever populated.

## When the subject is NOT CPO but competitors are

In the merged active comp set, some comps may be CPO and others not. `comp_stats.py`'s `channel_stats.primary_non_cpo` carves out CPO comps from the primary channel so the apples-to-apples comparison isn't polluted by a CPO cluster.

`primary_non_cpo` is emitted **only** when:
- The subject is non-CPO, AND
- `primary.n >= 2` (enough same-channel comps to be meaningful), AND
- `primary.cpo_count > 0` (there are CPO comps to carve out), AND
- After stripping CPO, the non-CPO slice still has `n >= 2` (C4 guard — rendering a 1-point "median" would mislead).

When the guard fires, `primary_non_cpo` is `null` and the Same-Channel View falls back to the full-primary median with a note.

```
Same-Channel View (Franchise)
  All Franchise comps:        median $27,900 (n=11, 4 CPO)
  Franchise non-CPO only:     median $26,400 (n=7)
  Your price vs non-CPO med:  +$1,100 (4.2%)
```

This prevents the false reading of "you're priced right against franchise comps" when the franchise comps are 40% CPO and your unit isn't.
