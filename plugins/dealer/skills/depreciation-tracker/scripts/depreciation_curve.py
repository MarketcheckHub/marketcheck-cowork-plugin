...
Reads a JSON config from stdin describing the per-period sold-summary aggregates
and emits the assembled curve: retention %, monthly rate, annualized rate,
acceleration verdict, and curve-shape classifier.

Anchor selection (per Q3=C):
  - When `msrp` is non-null AND > 0 → MSRP-anchored Retention %; emits
    `anchor_used="msrp"`. Each period's `retention_pct_msrp` is computed.
  - When `msrp` is null → falls back to prior-period anchor; emits
    `anchor_used="prior_period"`. `retention_pct_msrp` is omitted on every row.
  - Renderer reads `anchor_used` to choose its column header.

Acceleration verdict (per Q2=C — 5-band):
  Defined in `references/tier-and-verdict-bands.md`. Threshold scale: monthly
  depreciation rate (most-recent two consecutive periods).
...
Input (stdin JSON):
  {
    ...
    "msrp":        <float|null>,
    "anchor_mode": "msrp" | "prior_period" | "auto"
  }

Output (stdout JSON):
  {
    "ok": true,
    "anchor_used":      "msrp" | "prior_period",
    "anchor_value":     <float|null>,
...
Failure modes:
  ...
  - insufficient_periods — fewer than 2 priced periods (need at least 2 for
...
def main(argv: list[str]) -> int:
    ...
    rates_with_offset = [
        (p["monthly_rate_pct"], p["months_offset_from_today"])
        for p in out_periods if p["monthly_rate_pct"] is not None
    ]
