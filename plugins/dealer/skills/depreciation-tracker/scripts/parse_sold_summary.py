...
def _aggregate_multi_period(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    Used by the depreciation-tracker W1 curve workflow, which fires one
    `get_sold_summary` call per lookback period (current month, 60d ago, 90d
    ago, 6mo ago, 1yr ago). Each call's date_from/date_to typically spans a
    single month, but the server returns one row per (month, dimensions) tuple
    — so a multi-month window or multi-make rollup yields multiple rows.

    Aggregation: sum `sold_count`, weighted means of `average_sale_price`,
    `average_days_on_market`, and `average_msrp` (weight = sold_count).

    M8 divide-by-zero guard: when `total_sold_count == 0` for a month bucket
    (or overall), the corresponding `weighted_avg_*` fields emit `None` instead
    of NaN. Mirrors `_aggregate_state_baseline` discipline.
    """
