...
keyed on `make,model` and ranked by `price_over_msrp_percentage`) plus optional
volume context, and emits per-(make,model) parity status + cross-period
direction.

Status (per `references/tier-and-verdict-bands.md` parity-classifier rule):
