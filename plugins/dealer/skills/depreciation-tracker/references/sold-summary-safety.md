...
- **`inventory_type`** — `"Used"` or `"New"` (title case). Omitting silently
  defaults to `"New"` upstream — a used-vehicle workflow that omits this
  rolls up the wrong inventory type. Set per session `car_type`.
...
- **`summary_by`** — `"state"` for W1 W2 W3 W5 (single-state cuts); for W4
  also `"state"` (state is the rollup dimension itself). `"city_state"` only
...
## Multi-period date discipline (NEW for depreciation-tracker)
...
## Parameters to skip — same as golden
...
- **`dealer_type`** — DO NOT pass. Live-call testing showed
  ...
  intentionally omits dealer-type filtering from its input surface (per the
  approved plan).
...
depreciation-tracker workflows do NOT prompt the user for year** (this is
the Q1=A decision in the approved plan); the output renders a scope
qualifier:
...
renders the make-model curve and surfaces the all-years scope inline.
...
gap as a Key Signals warning.

## Aggregation in code

W1 uses `parse_sold_summary.py --aggregate-multi-period` (added by this
skill). Each call's output emits `multi_period_aggregate.{months,overall}`
which `depreciation_curve.py` consumes. Never hand-compute monthly weighted
means.
