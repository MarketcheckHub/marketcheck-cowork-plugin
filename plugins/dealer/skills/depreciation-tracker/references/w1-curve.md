...
- **No `year`** — `get_sold_summary` does not accept year. The curve aggregates
  ...
  template renders an explicit scope qualifier.
- **No subject VIN** required. W1 has no subject vehicle in the
  competitive-pricer sense.
...
1. `get_sold_summary` × 5 — one per period (`current`, `60d`, `90d`, `6mo`,
   ...
   get_sold_summary:
     ...
     inventory_type=<session.car_type>     (Used | New)
...
2. `search_active_cars` (rows=1) — finds a representative listing whose VIN
   ...
   Sort is `msrp desc` (not `price desc`) per Q3=C — picks a top-trim
   representative for a more reliable MSRP anchor. If the upstream rejects
   ...
   exposes `msrp` flat at the listing root (parse_search.py:118), so a decode
   may not even be needed when the rep listing carries MSRP. Wave B fires
   only when `parse_search.listings[0].msrp` is null.
...
`anchor_mode="prior_period"` (Q3=C); emit DQ event (a) for the failed decode
...
This is the Q3=C decision in the approved plan: skill always emits
prior-period retention (always renderable from the data); MSRP retention is
additive when the decode-or-rep-listing path supplies a non-null MSRP.
...
| `inventory_type=both` | profile preference | Halt-and-ask which to use. |
...
```
<make> <model> sold-90d median dropped from $<oldest_avg> to $<newest_avg> over <months_span> months
