...
Reference workflow. Multi-period sold-price trajectory + 5-band acceleration verdict + retention % (MSRP-anchored when available; prior-period anchor as fallback). Wave A fires 5 `get_sold_summary` calls (one per period) + 1 `search_active_cars` (rep-VIN); Wave B conditionally decodes the rep-VIN to extract MSRP. **Used-vehicle workflow** — halts on `default_inventory_type=new` and redirects to W5 (MSRP Parity), the symmetric New-only workflow. "Depreciation" / "value retention" vocabulary is Used-only; new vehicles haven't depreciated.
...
Per-make retention % over a 6-month window + 4-tier classification + tier-jump detection. Single-wave; 3 parallel `get_sold_summary` calls (current avg + prior avg + current volume). **Used-vehicle workflow** — halts on `default_inventory_type=new` and redirects to W5 (MSRP Parity). Residual value is a post-depreciation concept; new vehicles haven't reached residual yet.
