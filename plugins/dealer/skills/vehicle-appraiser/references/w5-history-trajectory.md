...
Triggered by "what's the price history on this VIN", "show me this VIN's listing trajectory", "how has this vehicle depreciated over time", "previous listings", "depreciation rate", "VIN history", etc. **US-only** — halt with *"Historical Trajectory workflow is US-only — `get_car_history` is not available for UK."* on a UK profile.

W5 is a **history viewer** workflow: surface the VIN's listing trajectory, dealer-hop pattern, sharp-drop and decertified red flags, total cumulative depreciation, and (optionally) a current ML fair-value prediction when the user is also evaluating a current trade-in offer. W5 does **not** compute appraisal band — for the comparable-backed value range, route the user to W1 (`/full-appraisal <VIN>`) once they have the current odometer.
...
1. The user explicitly requests fair-value or current-value guidance ("what's it worth now", "should I take this trade", "fair-value prediction") OR supplies `miles` up-front.
...
8. **Current ML Fair Value** (when step 3 ran — both PRIMARY and CONTEXT lines).
...
2. After Wave A returns, parse the user's intent:
   ...
   - Fair-value or current-value intent ("what's it worth now", "should I take this trade at $X") → check whether the user supplied current odometer.
...
| Step 3 invoked without `--miles` | User asks for fair-value but no current odometer | Halt with C4 prompt. |
| `cpo_ever is True AND current.is_certified is None` | Vehicle CPO in history; current row's is_certified field absent | Render the L5 caveat: *"⚠ This vehicle was CPO at one or more historical listings; the current listing's CPO status is unknown — confirm with the seller before pricing."* `decertified` flag does NOT fire (current is None, not explicitly False). |
