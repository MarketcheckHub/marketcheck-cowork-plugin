...
Two distinct concepts that the skill used to conflate. Handling them separately.
...
- `predict_price_with_comparables` — ~100KB envelopes on every real call (especially the dual-prediction W1 step 2 which fires this tool 2–4 times)
...
**Prescribed recipe — primary path.** Write the raw response to disk with the Write tool, then invoke the parser with `--file`. **All paths are scoped to `<session.run_id>`** — read it from the profile's `session` block (auto-assigned by `scripts/load_profile.py`); never hardcode `cpr-run` in agent flows.
...
**Alternative — `persist_response.py --content-file`.** If the raw response is already sitting on disk at some other path (e.g. a temp file from an earlier pass, or a deep-truncation subagent handed back a path), route it through the run directory with `persist_response.py`:
...
`persist_response.py --content-file` copies verbatim — no trim, no reshape. **Always pass `--run-id <session.run_id>`** so the file lands in the same scratch directory as the rest of the flow's intermediate files. The legacy default subdirectory `cpr-run` is preserved for backward-compat callers but is **concurrent-unsafe** — two simultaneous flows fighting over `cpr-run/asc.json` will silently swap responses (see the v1.4.1 concurrency fix). The script still accepts `--stdin` for bash-pipe callers; the stdin recipe remains functional for shell scripts outside the agent surface, but is not the path for agent workflows.

**Banned path: hand-keying listings into a custom merge script.** v4 hand-merged without a script (over-excluded via `comp_stats.exclude_vins`, n=21 vs 25). v5 hand-merged correctly but with no named script. v6 hand-keyed 30 listings into a new `build_merged.py` the agent invented on the spot. Every one of these paths drops programmatic VIN dedup and `--subject-vin` shadow detection. The asc/desc responses must flow through `parse_search.py --file` → `merge_comps.py`, regardless of response size — use the Write-tool recipe above to get bytes onto disk.
...
- Comp set call truncated → `"Competitive Set: N rows rendered from partial response; full set unavailable"`
- Sold-90d call truncated → omit the MoS line; log a Data Quality Notes event
