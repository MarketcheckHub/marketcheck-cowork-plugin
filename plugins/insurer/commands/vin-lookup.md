...
Full VIN lookup — decode the vehicle specs, pull its listing history, and estimate current market value. One command, complete picture for insurance claims context.

## Step 0: Load insurer profile

Read `~/.claude/marketcheck/insurer-profile.json`. If it exists, note the `location.zip` (US) or `location.postcode` (UK) and `location.country` for tool routing. If `country == UK`, inform the user: "VIN lookup (decode, history, price prediction) requires US data tools. Not available for UK vehicles."
