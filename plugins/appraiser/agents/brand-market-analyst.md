...
**Fastest depreciating statewide**: Call `get_sold_summary` with `state`, `inventory_type=Used`, `ranking_dimensions=make,model`, `ranking_measure=average_sale_price`, `ranking_order=asc`, `top_n=15`, `limit=5000` for current_month. Cross-reference with three_months_ago. → **Extract only**: make, model, average_sale_price per period.

**MSRP parity context**: Call `get_sold_summary` with `state`, `inventory_type=New`, `ranking_dimensions=make,model`, `ranking_measure=price_over_msrp_percentage`, `ranking_order=desc`, `top_n=10`, `limit=5000` for current_month. → **Extract only**: model, price_over_msrp_percentage. Note: new models selling below MSRP accelerate used vehicle depreciation for the same nameplate.
