...
MCP server (`mcp_server_tool_docs/get_sold_summary.md`: "US market only. No
UK variant.").
...
depreciation workflows can produce honest output, which means none of them
can produce defensible trend-adjusted valuations for a UK appraiser.
...
> does not expose sold-transaction aggregates, so the skill cannot build a
> defensible trend-adjustment for a UK valuation. For UK comparable-backed
> appraisals, use the `vehicle-appraiser` skill (UK adapts via
> `search_uk_active_cars` comp medians). Re-visit when MarketCheck ships UK
> sold data."*

## Why no UK adaptation here

Sibling skills that have UK adaptations (e.g., `vehicle-appraiser`) can
substitute `search_uk_*` listing data for the US-only tools at per-listing
granularity. Depreciation has no such substitute path:

- W1 curve needs **multi-period aggregated sold prices** —
  `search_uk_recent_cars` has at most 90 days of expired-listing data, not
  the 12-month curve W1 draws. And expired listings are not realised sales
  — they are just listings that left the active index (sold OR withdrawn OR
  transferred), so the appraisal-defense story breaks.
...
`vehicle-appraiser`. Until then, the halt message above is the entire UK
behavior.
