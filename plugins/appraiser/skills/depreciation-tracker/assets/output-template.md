# Output Template — Depreciation Tracker (Appraiser)
...
Specialization:  <appraiser.specialization>                            (when set)
Min comps:       <appraiser.min_comp_count>                            (when set)
```
...
Currency is always `$` (depreciation-tracker is US-only).
...
single-cut. Per the per-workflow refs:
...
- W1 — compare the subject's monthly rate to the W1 verdict-band median for
  ...
  segment average. When `appraiser.min_comp_count` is breached by any
  period's sold_count, surface a thin-data caveat naming the period.
- W2 — EV vs ICE explicit comparison; or top-vs-bottom segment delta.
- W3 — anchor on `appraiser.specialization` + top-volume make in
  `profile.location.state` per `references/w3-brand-residual.md`'s
  "Comparison Context (W3-specific)" section.
- W4 — compare profile state to national average + top-5 / bottom-5 spread.
- W5 — anchor on `appraiser.specialization` + top-volume make per
  `references/w5-msrp-parity.md`'s "Comparison Context (W5-specific)"
  section.

When the comparison axis is missing (e.g., no `specialization` in profile
for W3/W5), surface a Data Quality (g) event documenting the deferred
comparison and apply the default framing named in the per-workflow ref.
...
- W1: *"Annualized rate of 18.6% on this make/model exceeds the SUV segment
  average of 12.4% — apply ≥18%/yr trend discount against book values when
  this make is more than 60 days from book publication."*
- W2: *"⚠ Fleet revaluation note: EVs depreciating 1.8× faster than ICE in
  <STATE>; portfolios concentrated in EVs need EV-specific residual
  adjustment, not blended book."*
- W3: *"<make> dropped from T1 to T2 over 6 months (-2.3% retention); apply
  a 1–2% per-brand trend discount in current appraisals for this make."*
- W4: *"Adjacent <state> trades at 9% premium vs profile state — for a
  multi-state insurance claim or relocation appraisal, the destination-state
  anchor adds ~$<delta> per vehicle vs the source-state anchor."*
- W5: *"<model> deepening discount: now -4.2% vs -1.5% prior period —
  new-vehicle replacement-cost claims on this model should anchor on the
  prevailing transaction price (not MSRP) and cite the trend in the claim
  narrative."*
...
## Recommendation (always — sub-persona-tailored)

Tailor the recommendation to the appraiser's sub-specialty, drawing from
`references/outcomes.md` action-to-outcome funnel:

```
Trade-in:        <bid adjustment / per-brand trend discount based on the verdict>
Insurance:       <total-loss settlement anchor / replacement-cost implication>
Estate / Fleet:  <revaluation cycle / portfolio concentration implication>
```

Provide one sentence per sub-persona that applies. Quantify the business
impact:
- "1% monthly acceleration on a $30K vehicle = $300 of trend-discount per
  appraisal" (per outcomes.md line 1).
- "Every 1% retention error on a $30K vehicle = $300 of total-loss
  settlement variance" (per outcomes.md line 2).

When the profile's `specialization` is set, lead with that sub-persona's
recommendation; the other two are optional but recommended for breadth.
...
- **Any other country**: halt with the canonical US-only message named in
  the SKILL.md country-routing section. Re-evaluate when MarketCheck ships
  sold-summary data for additional markets.
