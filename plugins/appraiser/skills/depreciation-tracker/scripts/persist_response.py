...
Without `--run-id`, the script falls back to the legacy default "dt-run"
subdirectory — that path is **concurrent-UNSAFE** (two simultaneous flows
write to /tmp/marketcheck/dt-run/asc.json and silently swap responses).
...
Options:
  ...
  --run-id <id>             subdirectory under /tmp/marketcheck/ (default: dt-run; concurrent-unsafe)
"""
