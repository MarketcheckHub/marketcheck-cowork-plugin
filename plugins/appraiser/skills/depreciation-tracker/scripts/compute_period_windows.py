#!/usr/bin/env python3
"""
compute_period_windows.py — Emit calendar-month-aligned date windows for a
multi-period depreciation curve (W1 of the depreciation-tracker skill).
...
full set of windows in one shot — current month, plus N lookbacks — so the W1
caller never hand-computes any date.

Usage:
  compute_period_windows.py                                    # default periods
  compute_period_windows.py --today 2026-04-23                 # fixed today
...
Period semantics (each window is exactly one calendar month wide):
  current  → previous full month (date_to is end of last month)
  60d      → 2 months before current (offset = current_offset + 1)
...
month* — exactly what `compute_sold_summary_dates.py` calls "last 1 full
month."
