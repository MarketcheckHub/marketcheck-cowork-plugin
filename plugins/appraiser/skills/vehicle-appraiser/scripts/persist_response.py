...
Without `--run-id`, the script falls back to the default "appraisal-run"
subdirectory — that path is **concurrent-UNSAFE** (two simultaneous flows
write to /tmp/marketcheck/appraisal-run/asc.json and silently swap
responses). The skill flow always reads `session.run_id` from
`load_profile.py` and passes it explicitly; the default exists only as a
backstop for callers that have not yet wired up `session.run_id`.
...
Options:
  ...
  --run-id <id>             subdirectory under /tmp/marketcheck/ (default: appraisal-run; concurrent-unsafe)
"""
...
# Backstop default; concurrent-UNSAFE. Always pass --run-id explicitly
# (use session.run_id from load_profile.py) when running from the skill flow.
DEFAULT_RUN_ID = "appraisal-run"
