...
Triggered by "trade-in value", "what's it worth quick", "how much should I offer", "customer at the desk", etc. **Speed-pitch workflow** — the appraiser needs a credible number in under 25 seconds (the dealer is on the phone, the claim adjuster is on the call, the fleet manager is reviewing a batch).

W2 is **deliberately lighter than W1**. No desc tail-pull, no drop scan, no sold-90d stats trio, no `get_sold_summary`, no `get_car_history`, no Sold Transaction Comparables table. The user trades depth for throughput. For the formal report (insurance total-loss, estate, fleet revaluation) and full sold transaction evidence, route to W1 (`/full-appraisal <VIN>`).
...
A single up-front prompt before Wave A when the subject is a make/model with active CPO programs AND user has not pre-stated CPO status: *"Is this unit currently CPO?"*
...
predict_price_with_comparables(vin, miles, zip=<profile.zip>, dealer_type="franchise")          # parse → role: nocpo_franchise
predict_price_with_comparables(vin, miles, zip=<profile.zip>, dealer_type="independent")        # parse → role: nocpo_independent

[when subject is CPO]
predict_price_with_comparables(vin, miles, zip, dealer_type="franchise",   is_certified=true)   # role: cpo_franchise
predict_price_with_comparables(vin, miles, zip, dealer_type="independent", is_certified=true)   # role: cpo_independent
...
search_active_cars:
  ...
  zip=<profile.zip>, radius=<session.radius_mi_clamped>, car_type="used"   # HARDCODED — trade-in subjects are always Used by definition. The 78-85% wholesale-to-retail spread is a Used-car trade-in margin, so the comp set must also be Used.
...
parse_predict.py --file <predict-nocpo-franchise.json>     → nocpo_franchise role
parse_predict.py --file <predict-nocpo-independent.json>   → nocpo_independent role
[when CPO]
parse_predict.py --file <predict-cpo-franchise.json>
parse_predict.py --file <predict-cpo-independent.json>
...
The W2 card emits a **Recommended Trade-In Offer** range computed from the Predicted Retail Value (= `nocpo_franchise.marketcheck_price`) using a fixed retail-to-wholesale ratio.

The default ratio is **78–85% of franchise predicted retail** (industry rule-of-thumb). This default is fixed in W2's render template (`assets/w2-output-template.md`).

```
predicted_retail    = nocpo_franchise.marketcheck_price
predicted_wholesale = nocpo_independent.marketcheck_price
spread_$            = predicted_retail - predicted_wholesale
spread_pct          = spread_$ / predicted_retail * 100

offer_low  = 0.78 * predicted_retail
offer_high = 0.85 * predicted_retail
```

When `nocpo_franchise` predict is null (predict role degraded), render the offer range as `unavailable (predict role degraded; route to W1 for full appraisal)`. Do NOT synthesize a number from the active comp set — the spread analysis depends on the ML predict.
...
3. **CPO Premium** sub-block (when applicable) — `+$<premium>  (<pct>%)` per channel. No Net Margin line.
4. **Top 5 Retail Comparables** (8-col standard table, rows=5).
5. **Recommended Value (condition-adjusted)** — band block (compact form).
