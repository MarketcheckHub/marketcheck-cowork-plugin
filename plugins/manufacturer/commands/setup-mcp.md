...
Set up the MarketCheck MCP connection so all manufacturer plugin skills and commands can access automotive market data. Connects directly to MarketCheck's hosted MCP server — no npm packages or local processes required.
...
2. Run /onboarding to set up your manufacturer profile
3. Try: "Market share for Toyota in Texas" to verify it's working
4. Try: "Regional demand heatmap" for state-level intelligence
```
