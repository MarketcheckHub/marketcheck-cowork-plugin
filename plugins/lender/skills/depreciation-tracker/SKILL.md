...
2. If the file **exists**, use as optional context:
   ...
   - `tracked_segments` ← `lender.tracked_segments` — use as default segment filter if relevant
...
The primary user is a **lender** (residual value analyst, portfolio risk manager, or auto finance director) who needs to understand how quickly collateral values are declining to set accurate residual forecasts, adjust advance rates, and manage portfolio exposure. Secondary users include **lease residual setters** evaluating turn-in loss projections and **floor plan providers** monitoring collateral coverage on dealer inventory lines.

The following fields may be auto-filled from the lender profile:
...
|-----|-------------|-----------------|
| Monthly Depreciation Rate % | (Prior month avg price - Current month avg price) / Prior month avg price | Core residual risk metric; a 1% monthly acceleration on a $30K vehicle = $300/month additional exposure per loan |
...
| Brand Residual Ranking | Ranked list of makes by retention % with tier classification | Adjust advance rates by brand tier; Tier 4 brands warrant 5%+ lower advance rate caps |
| Price-Over-MSRP % | Transaction price / MSRP - 1, expressed as percentage | Positive values signal strong demand and lower near-term residual risk; negative values signal incentive-driven market and potential residual erosion |
| Geographic Value Variance | State price index (state avg / national avg x 100) | Apply state-level collateral value adjustments; using national averages in discount markets overstates collateral value |

## Action-to-Outcome Funnel

1. **Depreciation accelerating beyond 2% monthly on a specific model** — Residual risk increasing. Tighten advance rates on new originations for that model. For leasing portfolios, reduce residual value assumptions by the excess depreciation rate. For floor plan providers, shorten floorplan terms or reduce advance amounts. Cite the specific monthly rate and compare to the segment average.

2. **EV depreciation running 1.5x+ faster than ICE equivalent** — Apply separate residual curves for EV portfolio segments rather than using blended auto residuals. Lease residual settings for EVs should be 5-8% lower than ICE equivalents in the same segment. Consider requiring GAP insurance on EV loans with LTV > 90%. Show the EV vs ICE gap in dollars and percentage at each time interval.

3. **Brand drops from Tier 1 to Tier 2 retention** — Review portfolio concentration in that brand. Reduce advance rate caps by 2-3% for new originations. For existing portfolio exposure, flag loans on that brand's vehicles for early LTV monitoring. Show the trajectory over the prior 3-6 months to distinguish a blip from a trend.

4. **State-level price 10%+ above or below national average** — Apply state-level collateral value adjustments rather than national averages. For loans originated in discount markets, use the state-level value (not national) for LTV calculation to avoid overstating collateral coverage. Quantify the dollar impact per unit.

5. **New model flips from above-MSRP to below-MSRP** — Lower residual estimates for in-fleet units of that model. For lease portfolios with upcoming turn-ins on that model, increase loss reserves. For new originations, reduce advance rates to reflect the lower transaction price baseline. Show the timeline of the flip and the rate of discount deepening.
...
**Residual Risk Headline** — One sentence with the key finding (e.g., "The 2022 Toyota RAV4 has retained 87.3% of its original MSRP after 3 years, depreciating at 0.35% per month — residual risk is LOW, outperforming the SUV segment average of 0.52% monthly").

**Residual Risk Curve / Trend Table**

| Period | Avg Transaction Price | Retention % | Monthly Rate | Volume | Risk Signal |
|--------|----------------------|-------------|--------------|--------|-------------|
| Current Month | $XX,XXX | XX.X% | X.XX% | X,XXX | STABLE / INCREASING / DECREASING |
...
**LTV Impact** — If loan amount is known: "A $28,000 loan on this vehicle currently has an LTV of X%. At the current depreciation rate, LTV will reach 100% by [month] and 120% by [month]."
...
**Key Risk Signals** — Bullet list of notable trends:
- Acceleration or deceleration in residual risk (depreciation rate changes)
- Volume trends that may predict future price movements and residual pressure
- Geographic or segment-specific anomalies affecting collateral values
- MSRP parity shifts for new vehicles affecting used market residuals

**Recommendation** — One clear action tied to lending operations: adjust residual forecast, modify advance rate, change portfolio exposure limit, require GAP coverage, or apply geographic collateral adjustments. Include the quantified business impact (e.g., "Reducing advance rate by 3% on this model would reduce average exposure by $900 per loan on a $30K vehicle").
