...
1. Call `mcp__marketcheck__get_sold_summary` with:
   ...
   - `inventory_type`: `Used` (or `New` per user focus / profile default)
...
3. Also call with `ranking_order`: `desc` and `top_n`: `10` (plus `inventory_type` and `limit: 5000`) to get the **slowest** turning models.
   → **Extract only**: per make/model — `average_days_on_market`, `sold_count`. Discard full response.
