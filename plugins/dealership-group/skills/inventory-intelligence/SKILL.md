...
5. Confirm briefly: "Using location: **[location name]**, [State], [Country]"

## User Context

The following fields are loaded from the dealer group profile. Only ask if no profile exists:

- **Location**: Auto-loaded from profile (selected location's `state`)
- **Dealer ID**: Auto-loaded from profile (selected location's `dealer_id`)
- **Franchise brand(s)**: Auto-loaded from profile (selected location's `franchise_brands`)
...
- **Dealer type**: Auto-loaded from profile (selected location's `dealer_type`)

Do not guess dealer IDs or locations. If the selected location has `dealer_id: null`, ask before running lot-level workflows (Aging Inventory Alert, Category Gap).
...
|-----|-------------|-----------------|
| Demand-to-Supply Ratio | Ratio per make/model in the location's market | Identifies under-served segments; each correct stocking decision can add $1,500-3,000 in front-end gross |
...
4. **Scenario: Dealer group CFO asks "Where is our floor plan exposure highest?"**
   Run *Aging Inventory Alert* across multiple locations. Aggregate the carrying cost exposure by location. Rank locations by total aged-unit floor plan burn. Recommend priority actions for the top 3 most exposed stores.
