...
description: >
  ...
  rankings, MSRP-to-transaction price tracking for multi-location dealer groups.
version: 0.1.0
---
...
Load the `marketcheck-profile.md` project memory file. If exists, extract: `group_name`, `locations[]`, `preferences`; from default location: `state`, `franchise_brands`, `dealer_type`, `country`. If missing, ask for fields. US-only (`get_sold_summary` + `search_active_cars`); UK → not available. Confirm profile.
...
1. **Get current period sold data** — Call `get_sold_summary` with `make`, `model`, `inventory_type=Used`, `limit=5000`, `date_from` (first of prior month), `date_to` (end of prior month). Include `state` if specified.
   → **Extract only**: average_sale_price, sold_count. Discard full response.

2. **Get historical sold data at multiple intervals** — Make separate calls to `get_sold_summary` (each with `inventory_type=Used`, `limit=5000`) for each lookback period:
   - **60 days ago**
   - **90 days ago**
   - **6 months ago**
   - **1 year ago**
...
2. **Get prior period segment data** — Same call (with `inventory_type=Used`, `limit=5000`) with dates shifted back 3 months (or user's chosen comparison window).
   → **Extract only**: per body_type — average_sale_price, sold_count. Discard full response.

3. **Get fuel type comparison** — Call `get_sold_summary` with `fuel_type_category=EV`, current period dates, `inventory_type=Used`, `limit=5000`. Repeat with `fuel_type_category=ICE` (same `inventory_type`, `limit=5000`). Repeat both for prior period.
...
2. **Get prior period brand prices** — Same call (with `inventory_type=Used`, `limit=5000`) with dates shifted back 6 months.
   → **Extract only**: per make — average_sale_price. Discard full response.
...
2. **Get prior period parity data** — Same call (with `inventory_type=New`, `limit=5000`) with dates shifted back 3 months.
   → **Extract only**: per make/model — price_over_msrp_percentage. Discard full response.
