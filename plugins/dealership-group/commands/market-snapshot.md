...
- **If empty**: Check `~/.claude/marketcheck/dealership-group-profile.json` for default location state. If no profile, ask: "Which state? (e.g., TX, CA, FL)"

## Step 1.5: Load dealer group profile

Read `~/.claude/marketcheck/dealership-group-profile.json`. If it exists:
- Use the default location's state as the default if no state was specified
- Note the group name and locations for context
...
[If dealer group profile loaded: "Group: [Group Name] | Locations in this state: [list]"]
