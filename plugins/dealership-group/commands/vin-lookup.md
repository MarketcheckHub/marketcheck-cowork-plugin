...
Read `~/.claude/marketcheck/dealership-group-profile.json`. If it exists, note the default location's `zip` (US) or `postcode` (UK) and `country` for tool routing. If `country == UK`, inform the user: "VIN lookup (decode, history, price prediction) requires US data tools. For UK vehicles, use `/price-check` with year/make/model instead."
