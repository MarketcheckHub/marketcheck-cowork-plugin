---
description: Quick price check on a VIN or vehicle for a dealer group location
...
Read `~/.claude/marketcheck/dealership-group-profile.json`. If it exists:
- Determine which location to use (default or ask the user)
- Use the selected location's `zip` (US) or `postcode` (UK) as the default location — do not ask for ZIP
- Use the selected location's `dealer_type` as the default dealer type
- Note the location's `country` for tool routing
...
If no profile exists: "No dealer group profile found. Run `/onboarding` first." Then stop.
...
3. Use ZIP from location profile if available, otherwise ask
4. **US**: Call `mcp__marketcheck__predict_price_with_comparables` with:
   - `vin`: the VIN
   - `miles`: user's mileage
   - `zip`: location's zip code
   - `dealer_type`: from location profile or "franchise" (default)
...
VIN: [VIN]
Mileage: [X miles]  |  Location: [Location Name] ([Zip])
