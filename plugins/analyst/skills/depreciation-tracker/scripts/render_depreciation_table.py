...
depreciation-tracker workflows. Reads each script's pre-computed JSON output
and emits the canonical markdown table verbatim.

Four render modes (the dealer-side `geo` mode is intentionally absent — no
W4 in the analyst port):
...
VALID_MODES = ("curve", "segment", "brand", "parity")
...
def _render_curve(payload: dict[str, Any], currency: str) -> str:
    ...
    if verdict:
        extras.append(
            f"Verdict: **{verdict}**  "
            f"(most-recent monthly rate: "
            f"{_pct(_to_float(payload.get('recent_monthly_rate_pct')), signed=True, decimals=2)})"
        )
    shape = payload.get("curve_shape")
    if shape:
        extras.append(
            f"Curve shape: **{shape}**  "
            f"(longest-window monthly rate: "
            f"{_pct(_to_float(payload.get('longest_monthly_rate_pct')), signed=True, decimals=2)})"
        )
    if extras:
        lines.append("")
        lines.extend(extras)
