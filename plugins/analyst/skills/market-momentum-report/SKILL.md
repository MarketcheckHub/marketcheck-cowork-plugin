...
Repeat for prior month and 3 months ago (same parameters).
...
**EV penetration:** Call with `fuel_type_category=EV`, `inventory_type: New` (or `Used`), `limit: 5000` for current and prior. Calculate penetration rate and bps change.
...
- `ranking_order`: `desc`
- `top_n`: 25
- `inventory_type`: `New` (then repeat with `Used` — always set explicitly)
- `limit`: `5000`
